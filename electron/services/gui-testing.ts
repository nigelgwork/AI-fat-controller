...
// Test step types matching Windows MCP capabilities
...
export interface TestScenario {
  ...
  name: string;
  description: string;
  application?: string; // App to test (optional, may already be running)
  steps: TestStep[];
  createdAt: string;
  updatedAt: string;
}
...
function getTestsDir(): string {
  ...
  const testsDir = path.join(userDataPath, 'gui-tests');
...
async function executeStepViaMCP(
  ...
  try {
    switch (step.action) {
      ...
      case 'scroll': {
        // AutoIt uses mouse_wheel or similar
...
export async function runTestScenario(
  ...
  // Save result to disk
  const resultPath = path.join(getResultsDir(), `${scenario.id}-${Date.now()}.json`);
...
export function createTestScenario(scenario: Omit<TestScenario, 'id' | 'createdAt' | 'updatedAt'>): TestScenario {
  ...
  const filePath = path.join(getTestsDir(), `${newScenario.id}.json`);
...
export function getTestScenario(id: string): TestScenario | null {
  const filePath = path.join(getTestsDir(), `${id}.json`);
...
export function listTestScenarios(): TestScenario[] {
  const testsDir = getTestsDir();
  const files = fs.readdirSync(testsDir).filter(f => f.endsWith('.json') && !f.includes('-'));

  return files
    .map(f => {
      try {
        return JSON.parse(fs.readFileSync(path.join(testsDir, f), 'utf-8')) as TestScenario;
      } catch {
        return null;
      }
    })
...
export function getTestResults(scenarioId: string, limit?: number): TestResult[] {
  ...
  const results = files
    ...
    .map(f => {
      try {
        return JSON.parse(fs.readFileSync(path.join(resultsDir, f), 'utf-8')) as TestResult;
      } catch {
        return null;
      }
    })
