...
import * as mcpConfigRepo from '../db/repositories/mcp-config-repo';
import type { MCPServerConfig } from '../db/repositories/mcp-config-repo';

const log = createLogger('MCP');

// Re-export the config type from the repo
...
// Get the application version for MCP initialization
...
export class MCPServerManager {
  ...
  constructor() {
    // Load configs from database on construction
  }

  getConfigs(): MCPServerConfig[] {
    return mcpConfigRepo.list();
  }

  addConfig(config: MCPServerConfig): void {
    mcpConfigRepo.save(config);
  }

  removeConfig(name: string): void {
    mcpConfigRepo.remove(name);
  ...
  async connect(name: string): Promise<WindowsAutomationMCP> {
    const config = mcpConfigRepo.get(name);
    if (!config) {
  ...
  async autoConnectEnabled(): Promise<void> {
    const toConnect = mcpConfigRepo.getAutoConnect();
